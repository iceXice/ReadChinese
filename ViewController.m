...
@property (strong) IBOutlet NSButton *useEnglishAsKey;
...
//
        NSString*english = txt;
        if (_useEnglishAsKey.state) {
            [NSThread sleepForTimeInterval:0.2];
            english = [self youdao:txt];
            NSLog(@"翻译：  %@ = %@",english,txt);
            [dataMstr appendString:[[english stringByAppendingString:@"="] stringByAppendingString:                self.tradition.state ? [[ASConvertor getInstance] s2t:txt] : txt]];
        }
        else
        {
...
-(NSString*)youdao:(NSString*)string
{
    NSString *urlString = @"http://fanyi.youdao.com/translate";
    NSURL *url = [NSURL URLWithString:urlString];
    
    NSData *data = [Request bodyFromYoudaoWtihContent:string];
    
    Request *request = [[Request alloc] initWithUrl:url];
    request.delegate = self;
    request.HTTPBody = data;
    NSString *ret =  [request startSessionRequest:nil];
    return ret ;
